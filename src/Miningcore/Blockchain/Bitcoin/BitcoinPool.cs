...
public class BitcoinPool : PoolBase
{
    ...
    protected virtual async Task OnSubmitAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            logger.Info(() => $"[{connection.ConnectionId}] Share accepted: D={Math.Round(share.ShareDifficulty, 3)} | ND={Math.Round(share.NetworkDifficulty * coin.ShareMultiplier, 5)} | M={coin.ShareMultiplier}");
